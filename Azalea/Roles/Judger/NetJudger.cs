...
namespace Azalea.Roles.Judger
{
	public class NetJudger
	{
		...
		private static NetJudger instance;
		public static NetJudger Instance => instance;

        public NetJudger(ServerDetail hostDetail, string _name, string _identifier)
		{
			instance = this;
		...
		private async Task HostCommunication()
		{
			...
			while (IsRunning)
			{
				...
				NetCommand<JudgerCommandType> command;
				try
				{
					command = NetCommand<JudgerCommandType>.Unserialize(commandJson);
				}
		...
		private Task InvokeCommand(NetCommand<JudgerCommandType> command)
		{
